...
@pytest.mark.unit
def test_load_custom_beir_missing_text_field(tmp_path: Path) -> None:
    """Corpus entry without 'text' field raises ValueError."""
    import json

    data = {
        "queries": {"q1": "query"},
        "corpus": {
            "doc1": {"title": "Document without text"},
            "doc2": {"text": "Valid document"},
        },
        "qrels": {"q1": {"doc1": 1}},
    }
    beir_file = tmp_path / "missing_text.json"
    beir_file.write_text(json.dumps(data))

    with pytest.raises(ValueError, match="Corpus entry 'doc1' missing required 'text' field"):
        load_custom_beir(beir_file)


@pytest.mark.unit
def test_load_custom_beir_invalid_types(tmp_path: Path) -> None:
    """Non-dictionary types for required fields raise ValueError."""
    import json

    # Test queries as list
    data = {
        "queries": ["q1", "q2"],
        "corpus": {"doc1": {"text": "text"}},
        "qrels": {"q1": {"doc1": 1}},
    }
    beir_file = tmp_path / "invalid_queries_type.json"
    beir_file.write_text(json.dumps(data))

    with pytest.raises(ValueError, match="'queries' must be a dictionary"):
        load_custom_beir(beir_file)
    ...
    beir_file = tmp_path / "invalid_corpus_type.json"
    beir_file.write_text(json.dumps(data))

    with pytest.raises(ValueError, match="'corpus' must be a dictionary"):
        load_custom_beir(beir_file)
    ...
    beir_file = tmp_path / "invalid_qrels_type.json"
    beir_file.write_text(json.dumps(data))

    with pytest.raises(ValueError, match="'qrels' must be a dictionary"):
        load_custom_beir(beir_file)


@pytest.mark.unit
def test_load_custom_beir_file_not_found(tmp_path: Path) -> None:
    """Non-existent file raises FileNotFoundError."""
    nonexistent = tmp_path / "nonexistent.json"

    with pytest.raises(FileNotFoundError, match="Custom BEIR file not found"):
        load_custom_beir(nonexistent)


@pytest.mark.unit
def test_load_custom_beir_string_path(tmp_path: Path) -> None:
    """Function accepts both Path and string arguments."""
    import json
    ...
    beir_file = tmp_path / "custom.json"
    beir_file.write_text(json.dumps(data))

    # Test with string path
    result = load_custom_beir(str(beir_file))
    assert result["queries"]["q1"] == "query"

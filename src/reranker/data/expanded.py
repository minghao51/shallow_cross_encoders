...
def generate_expanded_pairs(
    ...
    """Generate expanded pair dataset with balanced relevance distribution.

    Strategy: For each query, pair it with docs from ALL topics (same and different domains)
    at appropriate relevance levels. This creates rich, balanced training data.
    """
    rng = random.Random(seed)
    records = []

    all_topics = [(d, t) for d, topics in DOMAIN_SEEDS.items() for t in topics]

    for domain, topics in DOMAIN_SEEDS.items():
        for topic in topics:
            query = topic["query"]

            # 1. Core pairs: this query with its own docs at each relevance level
            ...
            # 2. Same-domain hard negatives: other queries' docs in same domain
            # These share domain vocabulary but don't answer THIS query
            other_topics = [t for t in topics if t["query"] != query]
            for other_topic in other_topics:
                # Score=1: tangential (same domain but different topic)
                records.append(
                    {
                        "query": query,
                        "doc": other_topic["docs"][1],  # tangential doc
                        "score": 1,
                        "domain": domain,
                    }
                )
                # Score=0: irrelevant doc from same domain's other query
                records.append(
                    {
                        "query": query,
                        "doc": other_topic["docs"][0],  # irrelevant doc
                        "score": 0,
                        "domain": domain,
                    }
                )

            # 3. Cross-domain noise: irrelevant docs from different domains
            other_domain_topics = [
                (d, t)
                for d, topics_list in DOMAIN_SEEDS.items()
                for t in topics_list
                if d != domain
            ]
            # Sample a few to avoid explosion
            n_cross = min(4, len(other_domain_topics))
            for _, cross_topic in rng.sample(other_domain_topics, n_cross):
                records.append(
                    {
                        "query": query,
                        "doc": cross_topic["docs"][0],  # irrelevant doc
                        "score": 0,
                        "domain": domain,
                    }
                )

    # Now we have a base set. Let's analyze and balance it.
    by_score = {0: [], 1: [], 2: [], 3: []}
    for r in records:
        by_score[r["score"]].append(r)

    print(f"  Base dataset: { {k: len(v) for k, v in by_score.items()} }")

    # To balance: we need to add more score=2 and score=3 examples
    # Strategy: For each query, add more relevant docs by combining with docs from same domain
    # at higher relevance levels (but not the same topic)

    # Add more score=2 and score=3 by creating "medium relevance" pairs
    # Use docs from same domain but different queries at score=2 level
    for domain, topics in DOMAIN_SEEDS.items():
        for topic in topics:
            query = topic["query"]
            other_topics = [t for t in topics if t["query"] != query]

            # Add same-domain relevant docs as score=2 (related but not directly answering)
            for other_topic in other_topics:
                records.append(
                    {
                        "query": query,
                        "doc": other_topic["docs"][2],  # relevant doc from other query
                        "score": 2,
                        "domain": domain,
                    }
                )
    ...
    print(f"  After adding score=2: { {k: len(v) for k, v in by_score.items()} }")

    # Now balance by undersampling the majority classes to match the smallest
    # But we want roughly equal distribution, so let's target the middle
    target_per_class = min(len(by_score[2]), len(by_score[3]))

    balanced = []
    for score in [0, 1, 2, 3]:
        rng.shuffle(by_score[score])
        balanced.extend(by_score[score][:target_per_class])

    # If we need more to reach target, add more varied examples
    while len(balanced) < target_count:
        # Cycle through adding balanced examples
        for score in [0, 1, 2, 3]:
            if len(balanced) >= target_count:
                break
            # Pick a random query and pair with appropriate doc
            query_domain, query_topic = rng.choice(all_topics)
            query = query_topic["query"]

            if score in query_topic["docs"]:
                doc = query_topic["docs"][score]
            else:
                # Use irrelevant doc from another domain
                noise_domain, noise_topic = rng.choice(
                    [
                        (d, t)
                        for d, topics in DOMAIN_SEEDS.items()
                        for t in topics
                        if d != query_domain
                    ]
                )
                doc = noise_topic["docs"][0]
...
def generate_expanded_preferences(
    ...
    """Generate expanded preference dataset with balanced A/B preferences and hard pairs."""
    rng = random.Random(seed)
    records = []
    ...
    for domain, topics in DOMAIN_SEEDS.items():
        for topic in topics:
            ...
            # Create preferences between different relevance levels
            ...
            # Add hard negative comparisons: same-domain different-query docs
            other_topics = [t for t in topics if t["query"] != query]
            for other_topic in other_topics:
                ...
                # Hard pair: score 3 vs score 2 from same topic
                if rng.random() < 0.5:
                    records.append(
                        {
                            "query": query,
                ...
                # Hard pair: score 2 vs score 1 from same topic
                if rng.random() < 0.5:
                    records.append(
                        {
                            "query": query,
...
def generate_expanded_contradictions(
    ...
    """Generate expanded contradiction dataset with diverse subjects."""
    rng = random.Random(seed)
    records = []
    ...
    contradiction_templates = [
        ...
        },
        {
            ...
            "field": "storage_temp",
            "value_a": "-70°C required",
            "value_b": "2-8°C fridge",
        },
    ]
